...
const DOMAIN_QUESTIONS = {
  "Technical": [
    ...
    { id: "tech_workshop", type: "text", question: "If you are asked to organise a workshop or host a hackathon?? What would you plan??", placeholder: "Describe the idea, theme, or how would you make it a success..." },
...
function QuestionCard({ question, answer, onChange, onNext, onBack, canBack, isLast, domainLabel, winWidth }) {
  ...
  const canProceed = () => {
    ...
    if (question.type === "details_group") return question.fields.every(f => {
      const v = answer && answer[f.key] ? answer[f.key].toString().trim() : "";
      if (!v) return false;
          ...
          {question.fields.map((field, i) => (
            <div key={field.key} style={{
              ...
              {(!field.type || field.type === "text") && (() => {
                const val = (answer && answer[field.key]) || "";
                const isValid = (() => {
                  if (!val) return null;
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 3, flexShrink: 0, width: field.width || "160px" }}>
                  <input
                    ...
                    onFocus={e => { e.target.style.background = "rgba(201,168,76,0.06)"; }}
              ...
              {field.type === "radio" && (
                <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                  {field.options.map(opt => {
                    const selected = answer && answer[field.key] === opt;
                    return (
                      <button
                        key={opt}
                        onClick={() => {
                          const cur = answer && typeof answer === "object" ? answer : {};
                          onChange({ ...cur, [field.key]: opt });
                        }}
                        style={{
                          padding: "8px 14px",
                          background: selected ? "rgba(201,168,76,0.18)" : "rgba(255,255,255,0.03)",
                          border: selected ? "1px solid rgba(201,168,76,0.65)" : "1px solid rgba(255,255,255,0.1)",
                          borderRadius: 7,
...
const COMMON_QUESTIONS = [
  ...
  { id: "q_details2", type: "details_group", question: "A few more details", fields: [
    ...
    { key: "campus", label: "Campus", type: "radio", options: ["Sector-62", "Sector-128"] },
    { key: "scholar", label: "Scholar", type: "radio", options: ["Day Scholar", "Hosteler"] },
  ]},
    ...
    return (
      <div style={{ minHeight: "100vh", background: "radial-gradient(ellipse at 30% 40%, #0d0d1a 0%, #07070f 50%, #030308 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Cinzel', serif" }}>
        <div style={{ textAlign: "center", padding: "0 24px", animation: "slideIn 0.6s ease" }}>
          ...
          <p style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "1rem", color: "rgba(255,255,255,0.45)", lineHeight: 1.8, maxWidth: 420, margin: "0 auto 28px" }}>Thank you for applying to ZenCoders. We'll be in touch soon!</p>
          <a
            href="https://chat.whatsapp.com/CD9e9NJz2pJ0ShrLh0vSGj"
            target="_blank"
            ...
            style={{
              ...
              borderRadius: 8, padding: "12px 28px", marginBottom: 32,
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: winWidth < 768 ? "90px 16px 40px" : "100px 24px 60px", position: "relative", zIndex: 10 }}>
        <div style={{ width: "100%", maxWidth: 640 }}>
          <div style={{ marginBottom: 36, textAlign: "center" }}>
            ...
            <div style={{ width: 50, height: "1px", background: "linear-gradient(90deg, transparent, #C9A84C, transparent)", margin: "0 auto 18px" }} />
            <a
              href="https://chat.whatsapp.com/CD9e9NJz2pJ0ShrLh0vSGj"
              target="_blank"
              rel="noopener noreferrer"
              style={{
                ...
                letterSpacing: "0.1em", textTransform: "uppercase",
                boxShadow: "0 0 16px rgba(37,211,102,0.12)",
                transition: "all 0.25s",
              }}
              onMouseEnter={e => { e.currentTarget.style.background = "rgba(37,211,102,0.16)"; e.currentTarget.style.boxShadow = "0 0 24px rgba(37,211,102,0.25)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "rgba(37,211,102,0.08)"; e.currentTarget.style.boxShadow = "0 0 16px rgba(37,211,102,0.12)"; }}
            >
